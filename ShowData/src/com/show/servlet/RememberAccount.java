...
@WebServlet({"/account/remember-password"})
public class RememberAccount extends HttpServlet {
	@Override
			...
			try {
				...
				if(!user.getEmail().equals(email)) {
					...
					try {
						...
						String body = "Username:"+user.getId()+" Password:"+ user.getPassword()+" Fullname:"+user.getFullname()+" Vai trò:"+user.getAdmin();
...
//			            msg.setFileName(fileChooserString);
				...
				req.setAttribute("message", "Sai tên đăng nhập!"+e);
			}
		}
		req.getRequestDispatcher("/views/Official/quenMatKhau.jsp").forward(req, resp);
	}
}
